...
pit, tegenpit, nat, superroem — Zuid en Noord in kolommen. Daaronder hoe vaak
elke tactiek is toegepast, met een naam erbij (zie `TACTIEKEN.md`), de meest
gebruikte eerst.

Alle tellers waren er al in `KjState`; er hoefde niets aan de engine bij.

**De C#-kant staat klaar in de map `voor-windows/`:** drie nieuwe bestanden
(`Statistiek.cs`, `Tactieknamen.cs`, `StatistiekForm.cs`) en vier kleine
wijzigingen in bestaande bestanden, allemaal uitgeschreven in
`voor-windows/LEESMIJ.md`. De tactieknamen zijn met een scriptje uit de
Swift-bron overgezet, dus alle 67 nummers staan er en er is niets verlopen bij
het overtypen.

Let op: die bestanden zijn nooit gecompileerd — op de Mac staat geen .NET. De
opzet klopt, maar reken op wat schuurwerk bij de eerste build.
